...
type ILoggerDirectionsArgs = {
  [Property in keyof LoggerDirectionsType]?:
    ConstructorParameters<LoggerDirectionsType[Property]>[0]
}

type ILoggerDirectionsInstance = {
  [Property in keyof LoggerDirectionsType]?:
    InstanceType<LoggerDirectionsType[Property]>
}
...
export class Logger {
  ...
  private message(level: ILoggerMessage['level'], data: ILoggerMessage['data']) {
    if (this.disabled) {
      return
    }
